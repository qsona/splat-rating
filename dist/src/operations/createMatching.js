...
exports.calculateMatchingWithMinRateDiff = exports.createMatching = void 0;
const assert_1 = __importDefault(require("assert"));
const lodash_1 = require("lodash");
const prismaClient_1 = require("../prismaClient");
const season_1 = require("../models/season");
const createMatching = async (userId, channelId) => {
    // TODO: acquire lock
    ...
    const joinedUsers = await prismaClient_1.prisma.joinedUser.findMany({
        where: {
            roomId: room.id,
        },
    });
    ...
    const watchingUserRatings = [];
    (0, assert_1.default)(ratings.length >= 7);
    ...
    const joinedUsersSeparations = await prismaClient_1.prisma.joinedUsersSeparation.findMany({
    ...
    const teamsRatings = (0, exports.calculateMatchingWithMinRateDiff)(ratings, separations);
    const watchingUserIds = watchingUserRatings.map((r) => r.userId);
    const matching = await prismaClient_1.prisma.matching.create({
        data: {
            roomId: room.id,
    ...
    await prismaClient_1.prisma.room.update({
        where: { id: room.id },
        data: {
            watchedUserIds: {
...
const calculateMatchingWithMinRateDiff = (ratings, separations = []) => {
    assert_1.default.equal(ratings.length, 8);
    let minRateDiff = Number.POSITIVE_INFINITY;
    let rates = ratings.map((r) => r.mu);
    let totalRate = (0, lodash_1.sum)(rates);
    let currentAlphaTeamIndexes = [];
    const isSeparated = (alphaTeamIndexes) => {
        const alphaTeamUserIds = [];
        const bravoTeamUserIds = [];
        ratings.forEach(({ userId }, index) => (alphaTeamIndexes.includes(index) ? alphaTeamUserIds : bravoTeamUserIds).push(userId));
        return separations.every(({ firstUserId, secondUserId }) => {
            if (alphaTeamUserIds.includes(firstUserId) && alphaTeamUserIds.includes(secondUserId))
                return false;
            if (bravoTeamUserIds.includes(firstUserId) && bravoTeamUserIds.includes(secondUserId))
                return false;
            return true;
        });
    };
    for (let i = 1; i <= 5; i++) {
        for (let j = i + 1; j <= 6; j++) {
            for (let k = j + 1; k <= 7; k++) {
                ...
                if (!isSeparated(alphaTeamIndexes))
                    continue;
                const alphaTeamTotalRate = (0, lodash_1.sum)(alphaTeamIndexes.map((i) => rates[i]));
    ...
    const decidedBravoTeamIndexes = (0, lodash_1.difference)([0, 1, 2, 3, 4, 5, 6, 7], decidedAlphaTeamIndexes);
    return [decidedAlphaTeamIndexes, decidedBravoTeamIndexes].map((teamIndexes) => {
        return teamIndexes.map((index) => ratings[index]);
    });
};
exports.calculateMatchingWithMinRateDiff = calculateMatchingWithMinRateDiff;
//# sourceMappingURL=createMatching.js.map